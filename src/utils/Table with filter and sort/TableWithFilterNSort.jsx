...
const TableWithFilterNSort = ({ heading = [], children, loading = false, fetchData = () => { }, sort = "", setSort = () => { }, handleScroll = () => {},
    tableFilter = "", setTableFilter = () => { }, tableStyleExtra = {}
}) => {
    ...
    return (
        <div className={styles.mainConatiner} onScroll={handleScroll}>
            <table className={styles.table} style={tableStyleExtra}>
                <thead className={styles.tableHeader}>
                    <tr>
                        {heading.map((elemlent, index) => {
                            return (
                                <th
                                    style={(elemlent.showFilter || elemlent.showSort) ? { cursor: "pointer" } : {}}
                                    key={index}
                                            ...
                                            <img
                                                ...
                                                onClick={(e) => {
                                                    ...
                                                    if ((elemlent.showFilter || elemlent.showSort)) {
                                                        ...
                                                        const position = e.pageX + 200 >= window.innerWidth ?
                                                            { right: `${window.innerWidth - e.pageX + (window.innerWidth * .12)}px` } :
                                                            { left: `${e.pageX}px` }
                                                        ...
                                                        dispatch({
                                                            type: "SHOW", payload: {
                                                                style: {
                                                                    ...
                                                                    padding: "0",
                                                                    background: "#fff",
                                                                    color: "#000", borderRadius: ".5vw", fontSize: ".8vw", fontWeight: "550", boxShadow: "0px 0px 2px 2px #00000010"
                                                                }, info: <FilterBox setShowFilterBox={setShowFilterBox} showFilterBox={elemlent} fetchData={fetchData} sort={sort}
                                                                    setSort={setSort}
                                                                    tableFilter={tableFilter}
                                                                    setTableFilter={setTableFilter}
                                                                />
